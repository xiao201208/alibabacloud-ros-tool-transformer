...
- `--compatible`: Whether to use compatible mode when transforming Terraform to ROS template. If compatible, keep the
  ...
  `.mappings`, `.conditions`, `.rules` are retained by default, `--include-all-files `Parameter supports retaining all 
  files. Otherwise, it is transformed to a template using ROS syntax.
  **This option is only available for Terraform template files.**
- `--include-all-files`: Whether to retain all files when transforming Terraform to ROS template in compatible mode, 
  files only ending with `.tf`, `.tftpl`, `.tfvars`, `.metadata`, `.mappings`, `.conditions`, `.rules` are retained 
  by default
