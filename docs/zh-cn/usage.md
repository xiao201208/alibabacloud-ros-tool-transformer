...
- `--compatible`：Terraform 转换为 ROS 模板时是否使用兼容模式。 如果兼容，则将 Terraform 文件内容保留在生成的 ROS 模板中，默认仅保留以`.tf`、`.tftpl`、`.tfvars`、`.metadata`、`.mappings`、`.conditions`、`.rules` 结尾的文件，`--include-all-files`参数支持保留所有文件。 否则，转换为使用 ROS 语法的模板。**此选项仅适用于 Terraform 模板文件**。
- `--include-all-files`: 当使用兼容模式将 Terraform 转换为 ROS 模板时，是否保留所有文件，默认仅保留以`.tf`、`.tftpl`、`.tfvars`、`.metadata`、`.mappings`、`.conditions`、`.rules` 结尾的文件。
